...
for(args in 1:54){
  ...
  signal_data_mat <- NULL
  latent_data_mat <- NULL
  misp_data_mat <- NULL
  ...
  for(k in 1:B){
    ...
    signal_data_mat <- rbind(signal_data_mat, signal)
    ...
    latent_data_mat <- rbind(latent_data_mat, round(array(data), 2))
    
    
    ## Generating data for misspecified settings...
    #misp_data <- RFsimulate(model = RMstable(alpha = 2, scale = mis_sp),x=x,y=x,grid=TRUE,n=1)
    #misp_data_mat <- rbind(misp_data_mat, misp_data)
  }
  
  saveRDS(signal_data_mat,paste0("Simulation study",delim,"RandomFieldsData",delim,"signal_data K=",K," eta=",eta," C=",C," sig_sp=",sig_sp," start=",start,".rds"))
  saveRDS(latent_data_mat,paste0("Simulation study",delim,"RandomFieldsData",delim,"latent_data K=",K," eta=",eta," C=",C," sig_sp=",sig_sp," start=",start,".rds"))
  #saveRDS(misp_data_mat,paste0("Simulation study",delim,"Original Results",delim,"RandomFieldsData",delim,"misp_data K=",K," eta=",eta," C=",C," sig_sp=",sig_sp," start=",start,".rds"))
  cat(args, "\n")
}
